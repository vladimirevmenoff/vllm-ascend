...
def _make_pruned_compute_logits(lm_head, inv_map: torch.Tensor,
                                vocab_size: int):
    ...
    def compute_logits(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        pad = pad_cache.get(m)
        if pad is None:
            pad = torch.full((m, k + 1), torch.finfo(pruned.dtype).min,
                             dtype=pruned.dtype, device=pruned.device)
            pad_cache[m] = pad
...
def maybe_prune_lm_head(*models: object) -> None:
    ...
    for model in models:
        ...
        if lm_head is None:
            logger.warning("lm_head prune: no lm_head found on %s; skipped",
                           type(model).__name__)
            continue
        ...
        if scale != 1.0 or getattr(proc, "soft_cap", None):
            raise NotImplementedError(
                "lm_head pruning supports scale=1.0 / no soft cap only.")
        if id(lm_head) not in pruned_heads:
            ...
            lm_head.weight.data = maybe_trans_nz(
                pack["weight"].to(dev)).transpose(0, 1)
        ...
        logger.info(
            ...
            type(model).__name__, vocab_size, pack["weight"].shape[0])
